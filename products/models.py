...
# inventory.models.Supplier importi endi kerak emas
...
class Kassa(models.Model):
    ...
    class Meta: verbose_name = "Kassa/Filial"; verbose_name_plural = "Kassalar/Filiallar"; ordering = ['name']
    def __str__(self): return self.name

class Category(models.Model):
    # ... (o'zgarishsiz, is_phone_category olib tashlangan edi) ...
    name = models.CharField(max_length=100, unique=True, verbose_name="Nomi")
    description = models.TextField(blank=True, null=True, verbose_name="Tavsifi")
    barcode_prefix = models.CharField(max_length=10, unique=True, blank=True, null=True, validators=[RegexValidator(r'^[0-9a-zA-Z]*$', 'Faqat harf va raqamlar ruxsat etilgan.')], verbose_name="Shtrix-kod Prefiksi", help_text="Bu kategoriya uchun shtrix-kod boshlanadigan belgi(lar). Masalan: 'IPH', '01'.")
    is_accessory_category = models.BooleanField(default=False, verbose_name="Bu aksessuar kategoriyasimi?")
    class Meta: verbose_name = "Kategoriya"; verbose_name_plural = "Kategoriyalar"; ordering = ['name']
    def __str__(self): return self.name
...
class Product(models.Model):
    ...
    barcode = models.CharField(max_length=100, unique=True, blank=True, null=True, verbose_name="Shtrix-kod / IMEI")
    ...
    is_active = models.BooleanField(default=True, verbose_name="Aktiv")
    default_kassa_for_new_stock = models.ForeignKey(Kassa, on_delete=models.SET_NULL, null=True, blank=True, related_name='default_new_products_stock', verbose_name="Yangi mahsulot uchun standart kassa (omborga qo'shish uchun)")

    # supplier ForeignKey o'rniga yangi matnli maydonlar
    # supplier = models.ForeignKey(Supplier, ...) # BU OLIB TASHALDI (yoki avval qo'shilmagan bo'lsa, shart emas)

    # YANGI QO'SHILDI: Yetkazib beruvchi ma'lumotlari uchun matnli maydonlar
    supplier_name_manual = models.CharField(
        max_length=255, blank=True, null=True,
        verbose_name="Yetkazib Beruvchi Ismi (Qo'lda)"
    )
    supplier_phone_manual = models.CharField(
        max_length=20, blank=True, null=True,
        verbose_name="Yetkazib Beruvchi Telefon Raqami (Qo'lda)",
        validators=[RegexValidator(regex=r'^\+?1?\d{9,15}$', message="Telefon raqami to'g'ri formatda emas (masalan, +998901234567 yoki 901234567).")] # Ixtiyoriy validator
    )
    ...
    def save(self, *args, **kwargs):
        if self.barcode:
            ...
            if qs.exists(): raise ValidationError({'barcode': ['Bu shtrix-kod yoki IMEI allaqachon mavjud.']})
        super().save(*args, **kwargs)
    def __str__(self): return self.name
    class Meta: verbose_name = "Mahsulot"; verbose_name_plural = "Mahsulotlar"; ordering = ['name']
