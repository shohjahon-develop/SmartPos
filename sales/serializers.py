...
class SaleCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        contains_phone_product = False
        if items:  # items mavjudligini tekshirish
            for item_data in items:
                product = item_data.get('product_id')
                if product and product.category and product.category.is_phone_category:
                    contains_phone_product = True
                    break

        if contains_phone_product:  # Agar sotuvda telefon bo'lsa
            if not customer_id and not new_customer_data:
                raise serializers.ValidationError({
                    "customer_id": "Telefon sotilayotganda mijoz tanlanishi yoki yangi mijoz ma'lumotlari kiritilishi shart.",
                    "new_customer": "Telefon sotilayotganda mijoz tanlanishi yoki yangi mijoz ma'lumotlari kiritilishi shart."
                })
            if new_customer_data and not new_customer_data.get('phone_number'):
                raise serializers.ValidationError(
                    {"new_customer": {"phone_number": ["Yangi mijoz (telefon uchun) telefon raqami majburiy."]}})

        # Agar telefon sotilmayotgan bo'lsa va Nasiya bo'lsa, mijoz baribir majburiy
        ...
        # Umumiy tekshiruv (agar ikkalasi ham kiritilgan bo'lsa)
        if customer_id and new_customer_data:
            raise serializers.ValidationError("Faqat 'customer_id' yoki 'new_customer' dan biri kiritilishi mumkin.")
    ...
    def create(self, validated_data):
        ...
        customer_for_sale = customer_id_obj
        if not customer_for_sale and new_customer_data:  # Yangi mijoz kiritilayotgan bo'lsa
            phone = new_customer_data.get('phone_number')
            # Telefon raqami majburiyligi validatsiyada tekshirilgan,
            # lekin bu yerda ham tekshirish zarar qilmaydi, ayniqsa agar "telefon" sotilayotgan bo'lsa.
            customer_for_sale, _ = Customer.objects.get_or_create(phone_number=phone, defaults={
                ...
                'address': new_customer_data.get('address')})
        # Agar customer_for_sale hali ham None bo'lsa (va telefon sotilmayotgan bo'lsa), u None qoladi.
        # Sale modelida customer null=True, blank=True bo'lishi kerak.
